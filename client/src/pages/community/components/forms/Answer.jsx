...
const Answer = ({ question, questionId, authorId }) => {
    ...
    const generateAIAnswer = async () => {
        ...
        try {
            ...
            if (aiAnswer.reply) {
                ...
                alert("Sorry for the inconvenience. No reply from the server because all credits have been used.");
            }
            // alert(aiAnswer.reply);
    ...
    return (<div>
      ...
      <Form {...form}>
        <form className="mt-6 flex w-full flex-col gap-10" onSubmit={form.handleSubmit(handleCreateAnswer)}>
          <FormField control={form.control} name="answer" render={({ field }) => (<FormItem className="flex w-full flex-col gap-3">
                <FormControl className="mt-3.5">
                  <Editor apiKey={process.env.NEXT_PUBLIC_TINY_EDITOR_API_KEY} onInit={(evt, editor) => {
                // @ts-ignore
                editorRef.current = editor;
            }} onBlur={field.onBlur} onEditorChange={(content) => field.onChange(content)} init={{
                height: 350,
                menubar: false,
                plugins: [
                    "advlist",
