...
import { CallRecordingList, useCall, } from '@stream-io/video-react-sdk';
...
export const CallRecordings = () => {
    ...
    useEffect(() => {
        if (!call)
            return;
        const unsubscribeRecordingStopped = call.on('call.recording_stopped', () => setLoadingCallRecordings(true));
        const unsubscribeRecordingReady = call.on('call.recording_ready', (e) => {
            if (e.type !== 'call.recording_ready')
                return;
    ...
    return (<CallRecordingList CallRecordingListHeader={() => <div></div>} callRecordings={callRecordings} loading={loadingCallRecordings}/>);
};
