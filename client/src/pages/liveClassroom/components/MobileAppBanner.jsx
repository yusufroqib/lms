...
export const MobileAppBanner = (props) => {
    ...
    const platformLinks = {
        android: [
            {
                label: 'Try Android',
                url: `https://play.google.com/store/apps/details?id=io.getstream.video.android&referrer=${encodeURIComponent(`call_id=${callId}`)}`,
                active: true,
            },
    ...
    return (<div className="rd__try-native">
      <img className="rd__try-native__logo" alt="logo" src={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}/home.png`}/>
      ...
      {(platformLinks[platform] || [])
            ...
            .map((link) => (<a key={link.label} className="rd__button rd__button--primary" href={link.url} target="_blank" rel="noreferrer">
            <Icon className="rd__button__icon" icon="login"/>
            {link.label}
          </a>))}
      <button className="rd__try-native__use-browser rd__button rd__button--secondary" onClick={onDismiss}>
