...
export const ChatSendButton = ({ sendMessage, ...rest }) => {
    return (<div className="str-chat__send-button-container">
      <button aria-label="ArrowRightIcon" className="str-chat__send-button" data-testid="send-button" onClick={sendMessage} type="button" {...rest}>
...
export const ChatUI = ({ onClose, channelId, }) => {
    ...
    return (<Channel EmptyStateIndicator={NoMessages} SendButton={ChatSendButton} FileUploadIcon={() => <Icon icon="paperclip"/>}>
      <Window>
        <div className="rd__chat-wrapper">
          <div className="rd__chat-header">
            <h2 className="rd__chat-header__title">Chat</h2>
            <IconButton className="rd__chat-header__icon" onClick={onClose} icon="close"/>
          </div>
        </div>
        <MessageList messageActions={ALLOWED_MESSAGE_ACTIONS}/>
        <MessageInput focus additionalTextareaProps={{ placeholder: 'Send a message' }}/>
      </Window>
    </Channel>);
};
