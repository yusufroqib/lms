...
import { CallingState, CancelCallConfirmButton, Icon, LoadingIndicator, Notification, useCallStateHooks, useI18n, } from '@stream-io/video-react-sdk';
...
export const LatencyIndicator = () => {
    ...
    return (<div className="rd__header__latency">
      <div className={clsx('rd__header__latency-indicator', {
            'rd__header__latency-indicator--good': latency && latency <= 100,
            'rd__header__latency-indicator--ok': latency && latency > 100 && latency < 400,
...
export const Elapsed = ({ startedAt, }) => {
    ...
    const startedAtDate = useMemo(() => (startedAt ? new Date(startedAt).getTime() : Date.now()), [startedAt]);
    useEffect(() => {
    ...
    return (<div className="rd__header__elapsed">
      <Icon className="rd__header__elapsed-icon" icon="verified"/>
      <div className="rd__header__elapsed-time">{elapsed}</div>
    </div>);
};
export const ActiveCallHeader = ({ onLeave, selectedLayout, onMenuItemClick, }) => {
      ...
      <div className="rd__call-header__notifications">
        {(() => {
            if (isOffline || hasFailedToRecover) {
                return (<Notification isVisible placement="bottom" message={isOffline
            ...
            return (<Notification isVisible={isJoining || isReconnecting || isMigrating} iconClassName={null} placement="bottom" message={<LoadingIndicator text={isMigrating
                        ? 'Migrating...'
                        : isJoining
                            ? 'Joining...'
                            : 'Reconnecting...'}/>}>
